...
print(f"\nConnecting to traffic light at {TRAFFIC_LIGHT_IP}...")
try:
    traffic_light = TrafficLight(TRAFFIC_LIGHT_IP)
    status = traffic_light.status()
    print(f"✅ Traffic light connected! Status: {status}")
    print("Ensuring light is off...")
    traffic_light.off()
    time.sleep(1)
    print("✅ Traffic light ready for manual control")
except Exception as e:
    print(f"❌ Could not connect to traffic light: {e}")
    traffic_light = None
...
print("  R = Set light to RED")
print("  Y = Set light to YELLOW")
print("  G = Set light to GREEN")
print("  O = Turn light OFF")
print("=" * 70)
...
try:
    while True:
        ...
        if recvFlag and bytesReceived > 0:
            ...
            elif key == ord('r') and traffic_light:
                traffic_light.red()
                print('\n🔴 Traffic light set to RED')
            elif key == ord('y') and traffic_light:
                traffic_light.yellow()
                print('\n🟡 Traffic light set to YELLOW')
            elif key == ord('g') and traffic_light:
                traffic_light.green()
                print('\n🟢 Traffic light set to GREEN')
            elif key == ord('o') and traffic_light:
                traffic_light.off()
