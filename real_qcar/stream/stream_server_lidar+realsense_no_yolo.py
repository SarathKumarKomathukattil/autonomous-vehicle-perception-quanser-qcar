...
Autonomous Lane Following + RealSense Depth + LIDAR FUSION
- CSI Camera for RGB (lane detection only)
...
print("=" * 70)
print("=== AUTONOMOUS LANE FOLLOWING + STRICT SENSOR FUSION ===")
...
print("\n" + "=" * 70)
print("✅ READY - LANE FOLLOWING + STRICT SENSOR FUSION!")
...
def check_lidar_obstacle():
    ...
    - Strong detection: ≥20 points (filters signs/lights which give ≤14)
    ...
    # STRONG DETECTION: ≥20 points → Real obstacle (not sign/light)
...
def draw_rgb_display(frame, seg_mask, yellow_pos, round_pos, target_pos, mode):
    display = frame.copy()
    h, w = frame.shape[:2]
    ...
    display = cv2.addWeighted(status_overlay, 0.6, display, 0.4, 0)
...
try:
    while True:
        ...
        # Display
        rgb_display = draw_rgb_display(rgb_frame, seg_mask, yellow_pos, round_pos, target_pos, mode)
        cv2.imshow('CSI + STRICT FUSION', rgb_display)
