            ...
            while True:
                ...
                if success and image_data is not None:
                    try:
                        ...
                        # ✅ CREATE HSV LANE DETECTION WINDOW
                        if mode == "VISION":
                            ...
                            # Draw ROI bounds
                            roi_start_px, roi_end_px = roi_bounds
                            cv2.rectangle(hsv_vis, (0, roi_start_px), (w, roi_end_px), (0, 255, 0), 2)
                            
                            # Overlay cyan mask (in cyan color)
                            hsv = cv2.cvtColor(img, cv2.COLOR_BGR2HSV)
                            cyan_mask = cv2.inRange(hsv, self.lower_cyan, self.upper_cyan)
                            kernel = np.ones((5,5), np.uint8)
                            cyan_mask = cv2.morphologyEx(cyan_mask, cv2.MORPH_CLOSE, kernel)
                            cyan_mask = cv2.morphologyEx(cyan_mask, cv2.MORPH_OPEN, kernel)
                            cyan_overlay = cv2.merge([cyan_mask, cyan_mask, np.zeros_like(cyan_mask)])
                            hsv_vis = cv2.addWeighted(hsv_vis, 0.7, cyan_overlay, 0.3, 0)
                            
                            # Overlay white mask (in white/yellow color)
                            white_mask = cv2.inRange(hsv, self.lower_white, self.upper_white)
                            white_mask = cv2.morphologyEx(white_mask, cv2.MORPH_CLOSE, kernel)
                            white_mask = cv2.morphologyEx(white_mask, cv2.MORPH_OPEN, kernel)
                            white_overlay = cv2.merge([np.zeros_like(white_mask), white_mask, white_mask])
                            hsv_vis = cv2.addWeighted(hsv_vis, 0.7, white_overlay, 0.3, 0)
                            
                            # Draw detected cyan center
                            if cyan_pos:
                                cv2.circle(hsv_vis, cyan_pos, 15, (255, 255, 0), -1)
                                cv2.putText(hsv_vis, "CYAN CENTER", (cyan_pos[0]-50, cyan_pos[1]-20),
                                           cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 0), 2)
                            
                            # Draw detected white center
                            if white_pos:
                                cv2.circle(hsv_vis, white_pos, 15, (255, 255, 255), -1)
                                cv2.putText(hsv_vis, "WHITE CENTER", (white_pos[0]-55, white_pos[1]-20),
                                           cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 2)
                            
                            # Draw computed lane center
                            if lane_center:
                                cv2.circle(hsv_vis, lane_center, 20, (0, 255, 0), 3)
                                cv2.circle(hsv_vis, lane_center, 5, (0, 255, 0), -1)
                                cv2.putText(hsv_vis, "LANE CENTER", (lane_center[0]-60, lane_center[1]+40),
                                           cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 0), 2)
                                # Draw line from image center to lane center
                                cv2.line(hsv_vis, (w//2, h//2), lane_center, (0, 255, 0), 2)
                            
                            # Draw centerline
                            center_x = w // 2
                            cv2.line(hsv_vis, (center_x, 0), (center_x, h), (255, 0, 255), 1)
                            
                            # Add status text
                            cv2.putText(hsv_vis, f"Strategy: {strategy}", (10, 30),
                                       cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 2)
                            cv2.putText(hsv_vis, f"Cyan Pixels: {cyan_px}", (10, 60),
                                       cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 0), 1)
                            cv2.putText(hsv_vis, f"White Pixels: {white_px}", (10, 85),
                                       cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)
                            
                            # Resize and show
                            hsv_vis_resized = cv2.resize(hsv_vis, (615, 308))
                            cv2.imshow('Lane Detection (HSV)', hsv_vis_resized)
